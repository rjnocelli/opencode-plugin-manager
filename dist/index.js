...
import { findConfigPath, findGlobalConfigPath, readConfig, writeConfig, } from "./config.js";
import { resolveSource, installPlugin, uninstallPlugin, getPluginInfo, disablePlugin, enablePlugin, isUrlOrGitSource, } from "./plugin.js";
...
program
    ...
    .action(async (source, rawOptions, options) => {
    try {
        ...
        console.error(`Installed plugin: ${formatEntry(Object.keys(parsedOptions).length > 0
...
program
    ...
    try {
        ...
        console.log(`${"NAME".padEnd(namePad)}  SOURCE`);
        console.log("-".repeat(60));
        for (const plugin of plugins) {
            const status = plugin.disabled ? " (disabled)" : "";
            console.log(`${plugin.name.padEnd(namePad)}  ${plugin.source}${status}`);
            if (Object.keys(plugin.options).length > 0) {
                console.log(`${"".padEnd(namePad)}  options: ${JSON.stringify(plugin.options)}`);
            }
        }
    }
    catch (err) {
